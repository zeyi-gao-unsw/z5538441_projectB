...
## Index — how to read this log (46 sessions, 2026-07-29 → 2026-08-14)
...
| Deployment & finalisation (08-14) | 46 | Repo created and pushed, Streamlit Cloud deploy (incl. the private-repo permission fix), live app verified, links backfilled, report.pdf exported; includes the report-overwrite incident and its git recovery. |

Sessions 1–45 cover the build; Session 46 is the closing entry
...
finalise: my rewritten report (Word + exported PDF), the prompt-log index,
and the live links in the repo.
...
- I created the empty GitHub repository myself
  ...
  moved to my own browser - exactly as Appendix D intends (the final
  deploy needs the student's own login).
- Streamlit Cloud could not see the PRIVATE repository ("repository does
  not exist" is its permission error). I chose to flip the repo public
  early - the content was audited, no secrets and no raw data - rather
  than navigate GitHub's permission menus, and the deploy then succeeded.
- Kimi verified the live app end-to-end with Playwright screenshots: the
  landing renders and the Funds shelf loads the committed data
  (text-based locators fail on streamlit.app's wrapper frame, so
  verification was done by pixel screenshots).
- Links: I inserted the live URL + repo link into report Section 5 myself
  in Word and exported report/report.pdf; Kimi added both to README.md.

**A candid incident, recorded per the log rules.** During deployment prep
Kimi regenerated report.docx from the sample-build script, overwriting my
rewritten version in place. The version committed minutes earlier let us
restore it from git losslessly. Standing rule from this: report.docx is
the student's file and is never regenerated by the agent.

**Verification:** live app loads and is interactive at
https://zeyi-gao-unsw-z5538441-projectb-streamlit-app-hssehq.streamlit.app/;
report.pdf present; origin/main == local HEAD after the final push.

**Submission state:** the three deliverables are the Moodle zip (this
folder, minus .git/__pycache__/.DS_Store), the public repository link, and
the live app URL.
